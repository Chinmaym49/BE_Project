    ...
    if 'username' in session:
        if request.method == "POST":
            ...
            for i in range(len(tags_list)-1):
                q = "SELECT Question.id,Question.title from Question,QuesTag,Tag WHERE Question.id=QuesTag.qid AND QuesTag.tid=Tag.id AND Tag.tag='{}'".format(
                    tags_list[i])
                query = query+q+" UNION "
            q = "SELECT Question.id,Question.title from Question,QuesTag,Tag WHERE Question.id=QuesTag.qid AND QuesTag.tid=Tag.id AND Tag.tag='{}'".format(
                tags_list[-1])
            ...
            if duplicate_questions:
                ...
                return render_template('ask.html', session=session, duplicate_questions=duplicate_questions)
            else:
                cur = db.cursor()
                dop = datetime.now()
                print(dop)
                cur.execute('INSERT INTO Question(uid,title,body,dop) values(%s,%s,%s,%s)',
                            (session['id'], title, body, dop))
                qid = cur.lastrowid
                db.commit()
                cur.close()
                ...
                for id in tag_id:
                    cur = db.cursor()
                    cur.execute(
                        'INSERT INTO QuesTag(qid,tid) values(%s,%s)', (qid[0][0], id))
                    db.commit()
                    cur.close()
...
def questions(tag):
    ...
    if tag == "all":
        ...
        cur.execute(query)
        tid = cur.fetchone()[0]
        query = "select Question.id,Question.title from Question,QuesTag,Tag where Question.id=QuesTag.qid and QuesTag.tid=Tag.id and Tag.id={}".format(
            tid)
    cur.execute(query)
    ques = cur.fetchall()
    cur.close()
